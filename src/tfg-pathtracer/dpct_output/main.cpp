...
int main() {
	...
	*scene = demoScene2();
	...
	memset(pixelBuffer_8, 0, width * height * 4);
	...
	while (window.isOpen()) {
		...
		printf("\rkPaths/s: %f, %fGB of a total of %fGB used, %d/%d samples", ((float)sumPathcount(pathcountBuffer, width * height)), (float)(total - free) / (1024 * 1024 * 1024), (float)total / (1024 * 1024 * 1024), samples, sampleTarget);
