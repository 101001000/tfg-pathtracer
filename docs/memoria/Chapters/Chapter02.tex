...
Se acompaña el código hecho para la acumulación de muestras. \code{dev\_samples[]} es una variable global que almacena un contador con el número de muestras acumuladas hasta el momento para un píxel determinado.
...
La función \code{isnan} es necesaria para evitar errores numéricos al encontrar artefactos gráficos en determinadas escenas. 
...
\label{progressiverender}
	...
	\code{getBuffer} hace uso de la función \code{cudaMemcpyFromSymbolAsync} para realizar la copia asíncrona antes mencionada. También se hace copia del buffer de la suma de rayos emitidos por píxel con el fin de realizar métricas de eficiencia.
